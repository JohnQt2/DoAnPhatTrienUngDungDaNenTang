...
export default function WalletManageScreen() {
    const { wallets, syncPendingMutations, lastSyncError } = useStore();
    const [focusRefreshError, setFocusRefreshError] = useState<string | null>(
        null,
    );
            ...
            void syncPendingMutations().catch((error) => {
                ...
                const message =
                    error instanceof Error
                        ? error.message
                        : "Không thể đồng bộ dữ liệu ví.";

                setFocusRefreshError(message);

                if (__DEV__) {
                    console.warn(
                        "[WalletManageScreen] syncPendingMutations failed",
                        error,
                    );
                }
            });
    ...
    const totalBalance = useMemo(
        ...
        [focusRevision, wallets],
    );
    const syncWarning = lastSyncError || focusRefreshError;
                ...
                <View style={styles.totalCard}>
                    ...
                    {syncWarning ? (
                        <Text style={styles.syncWarning}>
                            Đang hiển thị dữ liệu cục bộ. Đồng bộ chưa hoàn
                            tất.
                        </Text>
                    ) : null}
                </View>
                    ...
                    {wallets.map((wallet, index) => (
                        ...
                        >
                            ...
                            >
                                ...
                                >
                                    <Ionicons
                                        name={getWalletIconName(wallet.icon)}
                                        size={24}
                                        color={wallet.color}
                                    />
                                </View>
